...
namespace ED
{
   ...
   **/
   void matching_augmentation (Graph & tree, NodeId const root_id, Graph & matching, NodeId const nodex_id, NodeId const nodey_id, std::vector<int> & levels, std::vector<size_type> &labels);
	...
	in the form that the first and last entry in circuit are in the same union and the edges in C are the edges between successive entries in circuit
	**/
	...
	//merges the unions of the vertices in circuit (unions are determined by iterating through labels), deletes the edges within the circuit from the matching
	...
	//necessary for unshrink_circuits
	bool size_comp(std::vector<ED::size_type> pair1, std::vector<ED::size_type> pair2);
	//iterates through the circuits in all_circuits, adds edges within these circuits to the matching
